...
export async function seedInitialData() {
  ...
  await db.transaction('rw', allTables, async () => {
    ...
    await db.documents.bulkAdd([
        { title: 'Old Safety Procedure', docNumber: 'SAF-001', revision: 1, owner: 'System', status: 'Published', nextReviewDate: new Date('2022-01-01'), tags: ['safety', 'procedure'] },
        { title: 'Current Quality Manual', docNumber: 'QM-002', revision: 2, owner: 'System', status: 'Published', nextReviewDate: new Date(new Date().setFullYear(new Date().getFullYear() + 1)), tags: ['quality', 'manual'] }
    ]);
    ...
    await db.checklistItems.bulkAdd([
      { checklistId, clause: '4.1', question: 'Has the org determined external/internal issues?', status: 'Pending' },
      { checklistId, clause: '4.2', question: 'Have interested party needs been determined?', status: 'Pending' },
    ]);
    ...
    await db.kpis.bulkAdd([
        ...
        { name: 'Complete Audits', objectiveId: 2, target: 10, value: 9, period: 'Q3 2025' },
    ]);
  });
}
