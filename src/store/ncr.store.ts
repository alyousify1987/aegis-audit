...
interface NcrState {
  ...
  isLoading: boolean;
  selectedNcr: INonConformance | null;
  fetchNcrs: () => Promise<void>;
  addNcr: (newNcr: Omit<INonConformance, 'id'>) => Promise<void>;
  selectNcr: (ncrId: number) => Promise<void>;
  clearSelectedNcr: () => void;
}
export const useNcrStore = create<NcrState>((set, get) => ({
  ...
  selectNcr: async (ncrId: number) => {
    set({ isLoading: true, selectedNcr: null });
    const ncr = await db.nonConformances.get(ncrId);
    set({ selectedNcr: ncr || null, isLoading: false });
  },
  clearSelectedNcr: () => {
    set({ selectedNcr: null });
  },
}));