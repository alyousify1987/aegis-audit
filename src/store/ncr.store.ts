...
export const useNcrStore = create<NcrState>((set, get) => ({
  ...
  fetchNcrs: async () => { set({ isLoading: true }); const ncrsFromDb = await db.nonConformances.toArray(); set({ ncrs: ncrsFromDb, isLoading: false }); },
  addNcr: async (newNcrData) => { await db.nonConformances.add(newNcrData as INonConformance); get().fetchNcrs(); },
  selectNcr: async (ncrId: number) => { set({ isLoading: true, selectedNcr: null }); const ncr = await db.nonConformances.get(ncrId); set({ selectedNcr: ncr || null, isLoading: false }); },
