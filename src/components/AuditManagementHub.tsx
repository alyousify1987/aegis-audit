  ...
  const handleOneClickAudit = async () => {
    try {
      ...
      await db.transaction('rw', db.documents, async () => {
        ...
        if (oldIngestedDocs.length > 0) { await db.documents.bulkDelete(oldIngestedDocs); }
      });
      ...
      for (const file of allFiles) {
        ...
        if (file.type.startsWith('image/') && ocrService.isReady) {
            try { extractedText = await ocrService.recognize(file); } catch (ocrError) { console.warn(`OCR failed for ${file.name}, proceeding without text.`); }
        }
        ...
        newDocuments.push({ title: file.name.split('.').slice(0, -1).join('.'), docNumber: `INGEST-${(file as any).fullPath}`, revision: 1, owner: 'Ingested', status: 'Draft', nextReviewDate: new Date(), tags: tags.length > 0 ? [...new Set(tags)] : ['ingested'], });
      }
      ...
      if ((err as Error).name === 'AbortError') { setIngestionStatus('Folder selection cancelled.'); } else { console.error('One-Click Audit failed:', err); setIngestionStatus('Ingestion failed. See console for details.'); }
      setTimeout(() => setIngestionStatus(''), 4000);
    }
  };
  return (
    <Paper elevation={3} sx={{ p: 3, borderRadius: 2 }}>
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 2, flexWrap: 'wrap' }}>
        ...
        <Button variant="outlined" color="primary" onClick={handleOneClickAudit}>'One-Click Audit' from Folder</Button>
        <Button variant="contained" color="secondary" onClick={handleSeedDatabase}>Seed Sample Data</Button>
        <Button variant="contained" color="error" onClick={handleClearDatabase}>Clear All Data</Button>
      </Box>
        ...
        <List>
          {audits.map((audit) => (
            <ListItem key={audit.id} divider>
              <ListItemText primary={audit.auditName} secondary={`Status: ${audit.status} | Scheduled for: ${new Date(audit.scheduledDate).toLocaleDateString()}`} />
