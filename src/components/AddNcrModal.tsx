...
interface AddNcrModalProps { open: boolean; onClose: () => void; }
...
export function AddNcrModal({ open, onClose }: AddNcrModalProps) {
  ...
  const handleSubmit = async () => {
    if (!ncrNumber || !processOwner) { alert('Please fill out all required fields.'); return; }
    const newNcr: Omit<INonConformance, 'id'> = { ncrNumber, classification, processOwner, status: 'Open', auditId: 1 };
    await addNcr(newNcr);
    onClose();
    setNcrNumber(`NCR-${Math.floor(100 + Math.random() * 900)}`); setClassification('Minor'); setProcessOwner('');
  };
  return (
        ...
        <Stack spacing={2} sx={{ mt: 2 }}>
          ...
          <FormControl fullWidth>
            ...
            <Select labelId="ncr-classification-label" id="ncr-classification-select" value={classification} label="Classification" onChange={(e) => setClassification(e.target.value as 'Minor' | 'Major')}>
              <MenuItem value="Minor">Minor</MenuItem><MenuItem value="Major">Major</MenuItem>
            </Select>
          </FormControl>
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1, mt: 2 }}><Button onClick={onClose} variant="outlined">Cancel</Button><Button onClick={handleSubmit} variant="contained">Save NCR</Button></Box>
        </Stack>
      </Box>
    </Modal>
  );
